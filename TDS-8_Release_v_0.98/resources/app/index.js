...
const OSC_LISTEN_PORT = parseInt(process.env.OSC_LISTEN_PORT || '8003', 10);  // M4L → Bridge default listener port (8003)
...
function requestReannounce(forceSweep = false) {
  ...
  setTimeout(() => { startTrackNameRequests(forceSweep); }, 300);
}
...
function requestNextTrackName() {
  ...
  try {
    // Send 0-31 to M4L (matches Live API 0-based indexing)
    sendOSC('/request-trackname', [{ type: 'i', value: trackRequestIndex }]);
    console.log(`📤 Requested track name for index ${trackRequestIndex} (0-based) from M4L`);
    trackRequestIndex++;
    ...
    if (!oscListener) {
        try {
            ...
            oscListener.on("message", (oscMsg, timeTag, info) => {
                ...
                // Treat active OSC traffic from M4L as a heartbeat as well
                // This lets the UI show M4L Connected even if the patch does not send /hi
                ...
                if (oscMsg.address === "/trackname" && oscMsg.args.length >= 2) { // Handle 2 or 3 args
                    ...
                    if (!Number.isFinite(at)) {
                      ...
                      if (Number.isFinite(di) && di >= 0 && di < 32) {
                        // Bridge sends 0-31, M4L echoes back 0-31
                        at = di;  // use displayIndex directly (0-31)
                      }
                    }
                    ...
                    // Ignore /activetrack 99 or any value outside 0-31
                    if (!Number.isFinite(index) || index < 0 || index >= 32 || index === 99) {
                      console.log(`⚠️ Ignoring /activetrack ${index} (out of range or 99)`);
                      return;
                    }
...
// -------- CRITICAL API ROUTES (Moved to Top) --------
app.get('/api/ports', async (_req, res) => {
  try {
    const ports = await listPorts();
    // Prefer real TDS-8 devices: XIAO ESP32-C3 (VID 303A, PID 1001)
    let tds8Devices = ports.filter(p => {
      const vid = (p.vendorId || '').toUpperCase();
      const pid = (p.productId || '').toUpperCase();
      return vid === '303A' && pid === '1001';
    });
    // If none found, show all
    const list = tds8Devices.length > 0 ? tds8Devices : ports;
    
    // Add connection status
    const decorated = list.map(p => {
      const isConnected = devices.some(d => d.path === p.path);
      const device = devices.find(d => d.path === p.path);
      return {
        ...p,
...
// Serve shared brand assets (logos, icons) from the assets directory
...
async function gracefulShutdown(reason) {
  ...
  try {
    try { console.log('\nShutting down TDS-8 Bridge...', reason || ''); } catch {}
    try {
      if (udpPort && typeof udpPort.close === 'function') {
        udpPort.close();
      }
    } catch {}
    try {
      if (oscListener && typeof oscListener.close === 'function') {
        oscListener.close();
      }
    } catch {}
    try {
...
app.get('/api/ports', async (_req, res) => {
  try {
    const ports = await listPorts();
    console.log('🔍 /api/ports - raw ports from SerialPort:', ports.map(p => ({
      path: p.path,
      vendorId: p.vendorId,
    ...
    // If no strict VID/PID matches, fall back to broader TDS-8 heuristics
    if (tds8Devices.length === 0) {
      console.log('⚠️ No strict VID/PID 303A:1001 matches; applying heuristic TDS-8 filter');
      tds8Devices = ports.filter(p => {
        const mfg = (p.manufacturer || '').toLowerCase();
        const lbl = (p.path || '').toLowerCase();
        const vid = (p.vendorId || '').toUpperCase();
        return (
          mfg.includes('arduino') ||
          mfg.includes('seeed')   ||
          lbl.includes('tds-8')   ||
          vid === '2886'         || // Seeed Studio
          vid === '2341'         || // Arduino
          vid === '1A86'            // Common CH340
        );
      });
    }

    if (tds8Devices.length === 0) {
      console.log('⚠️ No TDS-8-like ports found; returning empty TDS-8 device list');
      return res.json([]);
    }
    ...
    console.error('/api/ports error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/connect-all', async (_req, res) => {
  try {
    console.log('🔁 /api/connect-all requested from UI');
    await autoConnectDevices();
    res.json({ ok: true });
  } catch (e) {
    console.error('connect-all error:', e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/shutdown', (req, res) => {
  ...
  res.json({ ok: true });
  gracefulShutdown('Requested via /api/shutdown');
});
...
server.listen(PORT, () => {
  ...
  // Auto-connect scheduling disabled: devices will now only connect
  // when explicitly requested via the /api/connect endpoint from the UI.
  // This prevents ports like COM6 from being repeatedly auto-connected
  // and "stuck" without user control.
});
