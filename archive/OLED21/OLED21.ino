/**********************************************************************
  ...
  - First run w/o Wi-Fi: captive portal "TDS8-Setup" (pw: tds8setup)
...
void showAbletonConnectedAll(uint16_t ms = 1200);
...
void setup() {
  ...
  // If Wi-Fi is up but Ableton hasn’t sent anything yet, loop quick-start
  showQuickStartLoop();
}
...
void showNetworkSetup() {
  // 8 screens of simple guidance to join the AP and finish setup
  for (uint8_t i = 0; i < numScreens; i++) {
    tcaSelect(i);
    ...
    switch (i) {
      case 0: drawLines(2, 8, "SETUP", "Connect to", "TDS8-Setup"); break;
      case 1: drawLines(2, 8, "WiFi PW:", "tds8setup", ""); break;
      case 2: drawLines(2, 8, "Then open", "Browser", "tds8.local"); break;
      case 3: drawLines(2, 8, "Join your", "Wi-Fi and", "Wait ~20s"); break;
      case 4: drawLines(2, 8, "After join:", "Launch", "Ableton"); break;
      case 5: drawLines(2, 8, "Drag", "TDS8.amxd", "to new track"); break;
      case 6: drawLines(2, 8, "In device:", "Refresh", "Displays"); break;
      case 7: drawLines(2, 8, "Tip:", "Reannounce", "on Web UI"); break;
    }
    display.display();
  }
}
...
void showNetworkSplash(const String& ip) {
  // Make sure “TDS-8 ONLINE” always fits: if too wide at size=2, drop to 1
  for (uint8_t i = 0; i < numScreens; i++) {
    tcaSelect(i);
    display.invertDisplay(false);
    display.clearDisplay();
    ...
    if (w > (SCREEN_WIDTH - 6)) tsTitle = 1;

    drawCentered("TDS-8 ONLINE", tsTitle, 10);
...
void showAbletonConnectedAll(uint16_t ms) {
  for (uint8_t i = 0; i < numScreens; i++) {
    ...
    display.clearDisplay();
    display.drawRect(0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 1, SSD1306_WHITE);
    drawLines(2, 10, "Ableton", "Connected", "");
    display.display();
  }
  delay(ms);
}
...
void showQuickStartLoop() {
  ...
  const unsigned long maxLoopMs = 15000;  // don’t block forever; loop ~15s

  while (!abletonBannerShown && (millis() - start < maxLoopMs)) {
    // cycle four info screens across the strip
    for (uint8_t i = 0; i < numScreens; i++) {
      tcaSelect(i);
      display.invertDisplay(false);
      display.clearDisplay();
      ...
      switch (i % 4) {
        case 0: drawLines(2, 10, "Launch", "Ableton", ""); break;
        case 1: drawLines(2, 10, "Drag", "TDS8.amxd", "to new track"); break;
        case 2: drawLines(2, 10, "Hit", "Refresh", "Displays"); break;
        case 3: drawLines(2, 10, "Browser:", "tds8.local", ""); break;
      }
      display.display();
    }

    // Small breathing delay; break early if a name arrives
    for (int k=0; k<10; ++k) {
      delay(50);
      // if abletonBannerShown was set by /trackname, exit early
      if (abletonBannerShown) return;
    }
  }

  // After the loop, leave the last quick-start frames on screen
}
...
void handlePing(OSCMessage &msg) {
  // Default reply to the sender's source port; override if a port is provided
  uint16_t replyPort = Udp.remotePort();
  if (msg.size() >= 1) replyPort = (uint16_t)((int32_t)msg.getInt(0));
  IPAddress rip = Udp.remoteIP();

  // Reply immediately with /pong 1  (cast avoids ambiguous overload)
  OSCMessage m("/pong");
  m.add((int32_t)1);
  ...
  // Mark "connected" for ~4s; refresh Track 8 to show/hide the dot
  connectedUntil = millis() + 4000UL;
  if (!m4lConnected) {
    m4lConnected = true;
    drawTrackName(7, trackNames[7]);  // screen index 7 = Track 8
  }
}
