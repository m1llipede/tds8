...
if (WiFi.status() != WL_CONNECTED) {
  ...
  WiFi.disconnect(false, false); // keep any saved creds (don’t erase)
  ...
  if (ok) {
    // Persist what WiFiManager just connected to
    wifiSSID = WiFi.SSID();
    wifiPW   = WiFi.psk();
    saveWifiCreds();
    Serial.printf("Wi-Fi joined via portal: %s (saved)\n", wifiSSID.c_str());

    // optional: announce + restart discovery window
...
void loop() {
  ...
  // Handle UDP (ASCII first, else OSC)
  ...
  if (packetSize > 0) {
    ...
    if (n > 0 && isprint(buf[0])) {
      ...
      if (line.startsWith("PING")) {
        ...
        connectedUntil = millis() + 4000UL;   // refresh on every ping
        if (!m4lConnected) {
          m4lConnected = true;
    ...
    } // <-- IMPORTANT: close the ASCII block here
  ...
  // Heartbeat blink + timeout
  if (m4lConnected) {
    if (millis() > connectedUntil) {
      // heartbeat expired
      m4lConnected = false;
      hbOn = false;
      drawTrackName(7, trackNames[7]);   // remove the dot
    } else if (millis() >= hbNextToggle) {
      // flip the blink state
      hbOn = !hbOn;
      hbNextToggle = millis() + hbPeriod;
      drawTrackName(7, trackNames[7]);   // update Track 8
    }
  }
...
if (screen == 7 && m4lConnected && hbOn) {
  display.fillCircle(SCREEN_WIDTH - 6, 6, 6, SSD1306_WHITE);
}
