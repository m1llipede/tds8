...
// =====================  Demo film mode loop  ===================
#if DEMO_FILM_MODE
static void demoDrawDiagnostics() {
  // Simple diagnostic: draw borders and index numbers on all 8 screens
  for (uint8_t i = 0; i < numScreens; i++) {
    tcaSelect(i);
...
void loopDemo() {
  ...
  switch (demoState) {
    case DEMO_DIAG:
      ...
      if (now - demoStateStartMs >= 1000UL) { // 1 second
    ...
    case DEMO_REVEAL:
      // Reveal one more track every 200 ms
      if (demoTracksVisible < 8 && (now - demoLastRevealMs) >= 200UL) {
        demoTracksVisible++;
        demoLastRevealMs = now;
