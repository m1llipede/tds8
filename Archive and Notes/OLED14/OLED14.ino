...
void drawTrackName(uint8_t screen, const String& name) {
  ...
  display.setTextWrap(false);                    // avoid auto-wrap overlap
  ...
  const int M = 3;                 // margin from border so text doesn't touch
  const int maxW = SCREEN_WIDTH - 2 * M;
  const int charH1 = 8;            // default font height at textSize=1
  const int charW1 = 6;            // default font width incl. spacing at size=1
  ...
  // Fall back to size 1 if too wide
  if (w > maxW) {
    ts = 1;
    display.setTextSize(ts);
    display.getTextBounds(s, 0, 0, &x1, &y1, &w, &h);
  ...
  if (w <= maxW) {
    // Single-line centered with margin
    int x = M + (maxW - w) / 2;
  ...
  // Still too wide at size 1 → split into two lines
  // Compute max chars per line at ts=1
  int maxChars = maxW / (charW1 * ts);
  if (maxChars < 1) maxChars = 1;

  // Prefer to split at last space within maxChars; otherwise hard split
  int splitPos = -1;
  for (int i = 0; i < (int)s.length() && i < maxChars; ++i) {
    if (s[i] == ' ') splitPos = i;
  }

  String l1, l2;
  if (splitPos > 0) {
    l1 = s.substring(0, splitPos);
    int start2 = splitPos + 1;       // skip the space
    l2 = s.substring(start2);
  } else {
    l1 = s.substring(0, maxChars);
    l2 = s.substring(maxChars);
  }

  // If second line is still too long, trim with ellipsis
  if ((int)l2.length() > maxChars) {
    int keep = maxChars - 3;
    if (keep < 0) keep = 0;
    l2 = l2.substring(0, keep) + "...";
  }

  // Measure each line to center them with the margin
  display.setTextSize(ts);
  ...
  int lineH = charH1 * ts;
  int totalH = lineH * 2 + 2;      // 2px gap between lines
  int y0 = (SCREEN_HEIGHT - totalH) / 2;

  // Line 1
  int xL1 = M + (maxW - w1) / 2;
  display.setCursor(xL1, y0);
  display.println(l1);

  // Line 2
  int xL2 = M + (maxW - w2) / 2;
  display.setCursor(xL2, y0 + lineH + 2);
  display.println(l2);
