...
enum DemoState { DEMO_INTRO, DEMO_REVEAL, DEMO_HOLD };
DemoState demoState = DEMO_INTRO;
...
void loopDemo() {
  ...
  switch (demoState) {
    case DEMO_INTRO:
      ...
      if (now - demoStateStartMs >= 5000UL) { // 5 seconds
    ...
    case DEMO_REVEAL:
      // Reveal one more track every 60 ms
      if (demoTracksVisible < 8 && (now - demoLastRevealMs) >= 60UL) {
        demoTracksVisible++;
        demoLastRevealMs = now;
