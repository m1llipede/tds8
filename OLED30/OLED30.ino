...
// =======================  Firmware  =======================
#define FW_VERSION "0.30"   // ← bump this when you publish a new firmware
#define FW_BUILD "2025.10.12-wired"  // ← build identifier (date + feature)
...
void showWiredReady() {
  // Show "USB READY" on screen 7 (last screen)
  tcaSelect(7);
  display.invertDisplay(false);
  display.clearDisplay();
  display.setTextWrap(false);
  ...
  int16_t x1, y1; uint16_t w, h;
  display.getTextBounds("USB", 0, 0, &x1, &y1, &w, &h);
...
void handleSerialLine(const String& line) {
  ...
  if (cmd.startsWith("/trackname")) {
    ...
    if (sp1 > 0) {
      ...
      if (sp2 > 0) {
        ...
        if (idx >= 0 && idx < numScreens) {
          ...
          trackNames[idx] = name;
          drawTrackName(idx, trackNames[idx]);
          Serial.printf("OK: /trackname %d \"%s\"\n", idx, name.c_str());
        } else {
    ...
    Serial.println("ERR: Format: /trackname <idx> <name>");
    return;
  }
